  ...
  <div style="min-height:100vh; background:var(--dark); color:var(--text)">
    ...
    <div style="padding:28px;">
      ...
      <div v-else class="row g-4">
        <div v-for="product in filteredProducts" :key="product.id" class="col-sm-6 col-lg-4 col-xl-3">
          <div class="product-card" style="cursor:pointer;">
            <div style="width:100%; height:160px; overflow:hidden; background:#1a1a1a; border-bottom:1px solid #2a2a2a;">
              <img
              ...
              />
              <span v-else style="font-size:3rem; display:flex; align-items:center; justify-content:center; height:100%;">
                {{ categoryEmoji(product.category) }}
              </span>
            </div>
            <div class="product-card-body">
              <span class="badge bg-secondary bg-opacity-10 text-secondary mb-1" style="font-size:0.7rem;">{{ product.category }}</span>
              <h6 style="font-weight:700; margin:4px 0;">{{ product.name }}</h6>
              <p style="font-size:0.78rem; color:var(--gray); margin-bottom:10px;">{{ truncate(product.description, 60) }}</p>
              <div class="d-flex justify-content-between align-items-center mb-2">
                <span class="product-price">${{ Number(product.price).toLocaleString() }}</span>
                <span style="font-size:0.75rem; color:var(--gray);"><i class="bi bi-box me-1"></i>{{ product.stock }} disponibles</span>
              </div>
              <button class="btn btn-primary w-100" :disabled="Number(product.stock) === 0" @click="addToCart(product)">
      ...
      <div style="position:relative; width:380px; max-width:100%; background:var(--dark2); border-left:1px solid var(--border); display:flex; flex-direction:column; height:100vh; overflow:hidden;">
        <div style="padding:20px; border-bottom:1px solid var(--border); display:flex; align-items:center; justify-content:space-between;">
          <h5 style="margin:0; font-family:'Barlow Condensed',sans-serif; letter-spacing:1px;">
            <i class="bi bi-cart-fill me-2" style="color:var(--primary)"></i>MI CARRITO
          </h5>
        ...
        <div style="flex:1; overflow-y:auto; padding:16px;">
          ...
          <div v-for="item in cart" :key="item.id" style="display:flex; gap:12px; padding:12px 0; border-bottom:1px solid var(--border);">
            <div style="width:60px; height:60px; border-radius:8px; overflow:hidden; background:var(--dark3); flex-shrink:0; display:flex; align-items:center; justify-content:center; font-size:1.5rem;">
              <img v-if="item.image && item.image.startsWith('http')" :src="item.image" style="width:100%; height:100%; object-fit:cover;" />
            ...
            <div style="flex:1;">
              ...
              <div style="display:flex; align-items:center; gap:8px; margin-top:6px;">
                <button class="btn btn-outline-secondary btn-sm" style="width:28px; height:28px; padding:0;" @click="decreaseQty(item)"><i class="bi bi-dash"></i></button>
                <span style="font-weight:600; min-width:20px; text-align:center;">{{ item.qty }}</span>
                <button class="btn btn-outline-secondary btn-sm" style="width:28px; height:28px; padding:0;" @click="increaseQty(item)"><i class="bi bi-plus"></i></button>
                <button class="btn btn-outline-danger btn-sm ms-auto" style="width:28px; height:28px; padding:0;" @click="removeFromCart(item)"><i class="bi bi-trash-fill"></i></button>
              </div>
            </div>
          </div>
        </div>
        <div style="padding:16px; border-top:1px solid var(--border);">
          ...
          <button class="btn btn-primary w-100" style="padding:12px;" :disabled="cart.length === 0" @click="checkout">
            <i class="bi bi-lightning-charge-fill me-1"></i>Finalizar compra
          </button>
          <button class="btn btn-outline-secondary w-100 mt-2" @click="clearCart" :disabled="cart.length === 0">Vaciar carrito</button>
        </div>
      </div>
    </div>
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          <div class="modal-header border-0">
            <h5 class="modal-title"><i class="bi bi-check-circle-fill text-success me-2"></i>¡Compra realizada!</h5>
          </div>
          <div class="modal-body text-center" style="padding:30px;">
            <div style="font-size:3rem; margin-bottom:16px;">🎉</div>
            <h5>¡Gracias, {{ currentUser.value?.name }}!</h5>
          ...
          <div class="modal-footer border-0 justify-content-center">
            <button class="btn btn-primary" @click="confirmCheckout"><i class="bi bi-house me-1"></i>Seguir comprando</button>
          </div>
        </div>
      </div>
    </div>
...
function truncate(str = '', len = 60) { return str.length > len ? str.slice(0, len) + '…' : str }
...
function checkout() { showCart.value = false; showCheckout.value = true }
function confirmCheckout() { clearCart(); showCheckout.value = false }
function handleLogout() { authService.logout(); router.push('/login') }

onMounted(async () => {
  try { products.value = await productosService.getAll() }
